...
export default {
  ...
  async handler(this: MicroService, ctx: Context<Params>): Promise<any> {
    ...
    return this._find(ctx, { query: { email } }).then(([registeredEmail]:any) => {
      if (registeredEmail)
        return Promise.reject(new MoleculerClientError('Email already registered', 400));

      return this._find(ctx, { query: { username } }).then(([userNameTaken]:any) => {
        if (userNameTaken)
          return Promise.reject(new MoleculerClientError('Username already taken.', 400));
        return this._create(ctx, {
          ...
          return Promise.resolve(this.transformEntity(ctx, user, true, 'token'));
        });
      });
    });
  },
